...
const Chatbot: React.FC<ChatbotProps> = ({ generatedSpeech, onSpeechUpdate }) => {
  ...
  useEffect(() => {
    // This effect runs when the component mounts (i.e., when a new speech session starts via `key` prop)
    if (generatedSpeech) {
      setMessages([
        {
          role: 'model',
          text: 'Xin chào! Bạn muốn chỉnh sửa hay cải thiện điều gì trong bài phát biểu này không?',
        },
      ]);
        ...
        <div id="chatbot-content" className="border-t border-gray-200">
          ...
          <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200 flex items-center gap-2">
            <input
              ...
              placeholder="Hỏi AI để cải thiện bài phát biểu..."
